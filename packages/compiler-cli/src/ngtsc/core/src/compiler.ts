...
interface LazyCompilationState {
  ...
  contextCollector: ComponentContextCollector | null;
...
export class NgCompiler {
  ...
  private resolveCompilation(traitCompiler: TraitCompiler): void {
    this.perfRecorder.inPhase(PerfPhase.Resolve, () => {
      ...
      if (this.compilation?.contextCollector) {
        const data = this.compilation.contextCollector.getAll;
        if (data.length > 0) {
          const outputPath = resolve(
            this.adapter.getCurrentDirectory(),
            'src/app/generated-components.ts',
          );
          const outputDir = absoluteFrom(outputPath.substring(0, outputPath.lastIndexOf('/')));

          let imports = '';
          let mapEntries = '';
          const contextData = data.map((info) => {
            const componentName = info.name;
            const sourceFile = info.node.getSourceFile().fileName;
            let relativePath = relative(outputDir, absoluteFrom(sourceFile)).replace('.ts', '');
            if (!relativePath.startsWith('.')) {
              relativePath = `./${relativePath}`;
            }

            imports += `import { ${componentName} } from '${relativePath}';\n`;
            mapEntries += `  '${componentName}': ${componentName},\n`;
          ...
          const content =
            `// WARNING: This file is generated by the Angular compiler. Do not edit directly.\n\n` +
            ...
            `  required?: boolean;\n` +
            `}\n\n` +
            `const GENERATED_COMPONENT_CONTEXT_DATA: ComponentContext[] = ${JSON.stringify(
              contextData,
              null,
              2,
            )};\n\n` +
            ...
            `  { provide: COMPONENT_MAP, useValue: GENERATED_COMPONENT_MAP },\n` +
            `  { provide: COMPONENT_CONTEXT_DATA, useValue: GENERATED_COMPONENT_CONTEXT_DATA }\n` +
            `];\n`;

          this.adapter.writeFile(outputPath, content);
        }
      }
  ...
  private makeCompilation(): LazyCompilationState {
    ...
    const contextCollector = new ComponentContextCollector();
